...
import { ConversationReportModalPropsType } from "@/types";
import React, { useState } from "react";

const ReportModal = ({
  conversationId,
  handleModalClose,
}: ConversationReportModalPropsType) => {
  ...
  const handleSubmitReport = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/report-conversation/${conversationId}`,
        {
          method: "POST",
          headers: {
      ...
      setIsLoading(false);
      console.log("Report submitting error", error);
    }
  };
            ...
            {isSuccess && (
              <ModalLoadingContentWrapper>
                ...
                <span className="text-gray-600">
                  If any violence found. Admin will take necessary steps.
                </span>
              </ModalLoadingContentWrapper>
            )}
            {!isLoading && !isSuccess && (
              <ActionModalContentWrapper>
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-600 text-justify">
                  Experience any form of violence? Report it now for swift
                  action. Your safety matters. Review your concerns before
                  submitting as this is irreversible. We appreciate your
